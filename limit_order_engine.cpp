...
limit_order_engine::limit_order_engine(limit_order_book& order_book, server_codec& server_codec, client_codec& client_codec)
...
void limit_order_engine::remove_order(const order& _order, const std::string_view symbol) {
...
void limit_order_engine::on_request(limit_order::place_order_messasge& message, tcp::socket& _socket) {
    ...
    if (const auto order_placed = confirm_order(order_req, message.symbol); order_placed.has_value()) {
        ...
        write(_socket, asio::buffer(confirmed_data));
        ...
        write(_socket, asio::buffer(order_traded_data));
...
std::expected<limit_order_engine::order_placed, limit_order_engine::request_rejected> limit_order_engine::confirm_order(const order& _order, const std::string_view symbol) {
